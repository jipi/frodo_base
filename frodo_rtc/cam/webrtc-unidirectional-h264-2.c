...
<html> \n \
 ...
 \n \
      window.onload = function() { \n \
        ...
        var config = { 'iceServers': [{'urls': 'turn:turnhk.nong.be:5349','username': 'frodoturn_ph','credential': 'sambros2021'}] } \n \
 ...
 \n \
  <body> \n \
    <div> \n \
      <video width='100%' id=\"stream\" autoplay controls>Your browser does not support video</video> \n \
    </div> \n \
  </body> \n \
</html> \n \
";
...
create_receiver_entry (SoupWebsocketConnection * connection)
{
  ...
  receiver_entry->pipeline = gst_parse_launch ("webrtcbin name=webrtcbin stun-server=stun://" STUN_SERVER " "
      "nvarguscamerasrc ! video/x-raw(memory:NVMM), format=NV12, width=1640, height=1232 ! nvvidconv flip-method=2 ! nvvidconv ! video/x-raw(memory:NVMM), width=800, height=600 ! nvvidconv ! videorate ! video/x-raw, framerate=10/1 ! omxh264enc bitrate=1600000 peak-bitrate=2500000 control-rate=1 vbv-size=10 profile=1 ! video/x-h264, stream-format=(string)byte-stream ! queue max-size-time=100000000 ! h264parse ! "
